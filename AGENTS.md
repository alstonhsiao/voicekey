...
│   ├── main.py                 ← slim entry point（~344 行，所有邏輯移至 _voice_*.py）
│   ├── _voice_instance.py      ← 單例鎖 + PID 檔
│   ├── _voice_config.py        ← 設定載入、Mode / ModeManager、schema 驗證
│   ├── _voice_audio.py         ← AudioRecorder（sounddevice + NamedTemporaryFile）
│   ├── _voice_paste.py         ← beep / GCD 主執行緒 dispatch / paste_text
│   ├── _voice_postprocess.py   ← regex 修正、OpenCC 繁化
│   ├── _voice_providers.py     ← STT providers（Grok/OpenAI/Groq）+ CerebrasProvider
│   ├── _voice_session.py       ← SQLite session logger（chmod 600）
│   ├── _voice_menubar.py       ← rumps 選單列圖示
│   ├── _voice_hud.py           ← tkinter 浮動 HUD
│   ├── config.json             ← 設定（api / modes / hotkey / ui）
│   ├── requirements.txt        ← Python 依賴（已鎖版）
│   ├── install.sh              ← 自動安裝腳本
...
優先度 P2（中期）：
- [x] approach-3 封存：修 build.bat 移除 `--add-data config.json`（金鑰打包風險）；requirements.txt 鎖版（2026-06-12）
- [x] approach-6 main.py 拆 9 模組：`_voice_instance / _voice_config / _voice_audio / _voice_paste / _voice_postprocess / _voice_providers / _voice_session / _voice_menubar / _voice_hud`（2026-06-12）
- [x] `print()` 全改 `logging` 模組（main.py logging.basicConfig + FileHandler → `~/Library/Logs/WhisperVoice/app.log`）（2026-06-12）
- [x] config schema 驗證（缺欄位時給友善錯誤，而非 KeyError）（2026-06-12，`_voice_config.validate_config()`）
- [x] test_api_key.py / test_cerebras.py：API key 印出改為只印固定前綴 + 長度，不印任何秘密字元（2026-06-12）
- [x] install_manual.md 補 `opencc-python-reimplemented` 套件；同步 GROQ key 的 prompt_key（2026-06-12）
