...
> approach-7-xcode 一口氣執行期間記錄的待處理問題。
...
- 類型：需真人操作
- 現況：`xcodebuild build` 成功產出 `WhisperVoice.app`，`@main` + `NSStatusItem` 顯示 🎤、含「❌ 結束程式」。Headless 無法目視選單列圖示。
- 暫時處理：build 成功 + .app 產出即視為自動驗證通過。
- 待辦：使用者執行 `open ~/Library/Developer/WhisperVoice-DD/Build/Products/Debug/WhisperVoice.app`，確認選單列右上出現 🎤，點開有「❌ 結束程式」。
...
- 暫時處理：固定 DerivedData 到 `~/Library/Developer/WhisperVoice-DD`（非同步目錄），build/test 一律經 `build.sh` / `test.sh`。已解決，不需使用者處理。
...
- 待辦：使用者首次啟動 app 時於對話框允許「麥克風」；按 Ctrl+F1 講一句、再按一次停止，確認 `~/Library/Logs/WhisperVoice/app.log` 出現「💾 WAV 已存…」且該 WAV 為 16k mono。
...
- 待辦：①真人按 Ctrl+F1 講話，確認文字「貼到游標處」、狀態列即時切換 🔴→🔄→⏸。②**重要**：雙擊啟動的獨立 `WhisperVoice.app` 是自己的責任程序，需在「系統設定 → 隱私權與安全性 → 輔助使用 / 麥克風」**各自**勾選 WhisperVoice（與從終端機繼承不同）。
