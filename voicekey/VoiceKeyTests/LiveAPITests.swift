...
final class LiveAPITests: XCTestCase {
    ...
    private var enabled: Bool {
        ...
        let sentinel = FileManager.default.homeDirectoryForCurrentUser
            .appendingPathComponent("Library/Application Support/VoiceKey/.run_live_tests")
        return FileManager.default.fileExists(atPath: sentinel.path)
    }

    func testLiveGrokThenCerebras() async throws {
        try XCTSkipUnless(enabled, "create ~/Library/Application Support/VoiceKey/.run_live_tests to run live API tests")
        ...
        let phrase = "Hello, this is a VoiceKey native build test."
