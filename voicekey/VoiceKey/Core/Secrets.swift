...
enum Secrets {
    ...
    static func loadEnv() -> [String: String] {
        ...
        var candidates: [URL] = []
        // VOICEKEY_ENV_FILE preferred; WHISPERVOICE_ENV_FILE kept for pre-rename setups.
        if let custom = result["VOICEKEY_ENV_FILE"] ?? result["WHISPERVOICE_ENV_FILE"], !custom.isEmpty {
    ...
    private static let keychainService = "com.alston.VoiceKey"
    /// Pre-rename service name; read-only fallback so stored keys keep working.
    private static let legacyKeychainService = "com.alston.WhisperVoice"

    static func keychainRead(account: String) -> String? {
        keychainRead(service: keychainService, account: account)
            ?? keychainRead(service: legacyKeychainService, account: account)
    }
