...
enum AppPaths {
    ...
    /// (first access migrates files from the pre-rename WhisperVoice dir)
    static let appSupport: URL = {
        let fm = FileManager.default
        let url = fm.homeDirectoryForCurrentUser
            .appendingPathComponent("Library/Application Support/VoiceKey", isDirectory: true)
        try? fm.createDirectory(at: url, withIntermediateDirectories: true)
        migrateLegacyDir(into: url)
        return url
    }()

    /// Copy vocab files / env.local / config.local.json from the legacy
    /// "WhisperVoice" App Support dir. Never overwrites files already in the
    /// new dir, so it is safe to run on every launch.
    private static func migrateLegacyDir(into url: URL) {
        let fm = FileManager.default
        ...
        guard let items = try? fm.contentsOfDirectory(at: legacy, includingPropertiesForKeys: nil) else {
            return
        }
        for src in items {
            ...
            AppLog.info("📦 已從 WhisperVoice 舊目錄遷移：\(src.lastPathComponent)")
        }
    }
