...
```bash
# 1. 進入方案六目錄
cd /path/to/voicekey/approach-6-whisper-macos
...
API Key 存放在專案**根目錄**（`voicekey/`）的 `env.local` 檔案中。
...
```bash
# 用文字編輯器開啟
open -e /path/to/voicekey/env.local
```
...
🎤 Whisper 語音轉文字工具已啟動（macOS）
   ...
   Provider：grok
   目前模式：📝 直接轉錄
   結束：選單列 ❌ 結束程式 或 Ctrl+C
==================================================
```

同時 macOS **選單列右上角**出現 🎤 圖示，點選可切換模式或結束程式。
...
**輔助使用（Accessibility）與輸入監控（Input Monitoring）是兩個不同的權限：**

| 權限 | 用途 | 少了會怎樣 |
|---|---|---|
| 輸入監控 | 偵測你按了哪個鍵 | 熱鍵沒反應 |
| **輔助使用** | **模擬鍵盤送出 Cmd+V** | **辨識成功但文字不會自動貼上** |

**解法：**
1. 系統設定 → 隱私權與安全性 → **輔助使用**
2. 確認 **Terminal**（或你啟動程式的 App）已打勾
3. 授權後**重新啟動程式**才會生效

> Terminal 印出 `不允許「osascript」傳送按鍵。(1002)` → 就是這個權限未開。
...
```bash
# 刪除虛擬環境（最大的部分）
rm -rf /path/to/voicekey/approach-6-whisper-macos/.venv

# 刪除暫存錄音檔（系統 tmp，通常重開機會自動清除）
rm -f /tmp/whisper_voice_mac.wav

# 刪除啟動捷徑（如果建立了）
rm -f /path/to/voicekey/approach-6-whisper-macos/啟動語音輸入.command
